...
function HorizontalContent({ children }: { children: React.ReactNode }) {
  const { activeStep, previousActiveStep } = useStepper();
  ...
  React.useEffect(() => {
    if (contentRef.current && previousActiveStep !== undefined) {
      contentRef.current.style.transform = `translateX(${-100 * activeStep}%)`;
    }
